  ...
  return (
    <form onSubmit={submit} className="section-block">
      <div className="form-grid">
        <label>Customer reference<input name="customer_reference" value={form.customer_reference} onChange={(event) => update(event.target.name, event.target.value)} required /></label>
        ...
        <div className="full"><button className="button primary" type="submit" disabled={loading}>{loading ? "Running" : "Run recommendation"}</button></div>
      </div>
      {error ? <p className="error" role="alert">{error}</p> : null}
      {result ? <section className="result" aria-live="polite"><p className="eyebrow">Recommendation</p><div className="result-product">{title(result.recommended_product)}</div><div className="content-grid" style={{ marginTop: 16 }}><div><strong>Confidence</strong><br />{(result.confidence * 100).toFixed(1)}%</div><div><strong>Model</strong><br />{result.model_version}</div></div><h3>Reason codes</h3><ul className="plain-list">{result.reason_codes.map((reason) => <li key={reason}>{reason}</li>)}</ul><div className="notice warning" style={{ marginTop: 18, marginBottom: 0 }}><strong>Human review required</strong> The salesperson remains responsible for the final offer decision.</div></section> : null}
    </form>
  );
}
